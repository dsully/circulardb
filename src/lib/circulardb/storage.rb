module CircularDB
  ...
  class Storage
    ...
    [:name, :filename, :max_records, :min_value, :max_value, :interval, :units, :type, :num_records].each do |meth|
      class_eval <<-METH
        def #{meth}; @header[:#{meth}]; end
      METH
    end

    [:name, :max_records, :min_value, :max_value, :interval, :units, :type].each do |meth|
      class_eval <<-METH
